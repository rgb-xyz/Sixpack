...
static void test() {
    ...
    const auto run = [&](const double r) {
        ...
        if (--activeTasks == 0) {
            ...
            std::cout << std::format("Last result [r = {:f}, phi -> 360deg, theta -> 180deg]:", r) << std::endl;
            std::cout << "g" << std::endl;
            for (int j = 0; j < 4; ++j) {
                for (int i = 0; i < 4; ++i) {
    ...
    String speed = std::format("{}", int64_t(double(4i64 * R_STEPS * PHI_STEPS * THETA_STEPS) / seconds));
    for (StringPosition i = speed.size() - 3; i < speed.size(); i -= 3) {
        speed.insert(i, "'");
    }
